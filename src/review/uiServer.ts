...
type UiReviewAction = "approved" | "rejected" | "revision_requested" | "held" | "apply" | "apply_approved" | "discard" | "revert_previous" | "revert_initial" | "remaining_approved" | "remaining_rejected";
type UiReviewDecision = { index?: number; action: UiReviewAction; reasonTag?: string; comment?: string };
...
async function handleRequest(request: http.IncomingMessage, response: http.ServerResponse, data: ReviewUiData, requestToken: string) {
  ...
  if (request.method === "POST" && url.pathname === "/submit") {
    ...
    try {
      ...
      const comment = buildUiReviewComment(decisions);
      if (DRY_RUN) {
...
function validateDecisions(value: unknown, articleCount: number): UiReviewDecision[] {
  ...
  const allowedActions = new Set<UiReviewAction>(["approved", "rejected", "revision_requested", "held", "apply", "apply_approved", "discard", "revert_previous", "revert_initial", "remaining_approved", "remaining_rejected"]);
  ...
  return value.map((item) => {
    ...
    if (["revision_requested"].includes(decision.action) && !decision.comment?.trim()) throw new Error(`${decision.index}番の修正指示を入力してください`);
    return { ...decision, comment: decision.comment?.trim().slice(0, 1000) || "" };
  });
}

function buildUiReviewComment(decisions: UiReviewDecision[]) {
  return decisions.map((decision) => {
    if (decision.action === "remaining_approved") return "残り採用";
    if (decision.action === "remaining_rejected") return ["残り却下", decision.reasonTag, decision.comment?.trim()].filter(Boolean).join(" ");
    if (!Number.isInteger(decision.index) || (decision.index || 0) < 1) throw new Error("記事番号が不正です");
    const prefix: Record<UiReviewAction, string> = {
      approved: "採用", rejected: "却下", revision_requested: "修正", held: "保留",
      apply: "適用", apply_approved: "適用 採用", discard: "やめる", revert_previous: "一つ前に戻す", revert_initial: "初版に戻す",
      remaining_approved: "残り採用", remaining_rejected: "残り却下"
    };
    return [`${decision.index} ${prefix[decision.action]}`, decision.reasonTag, decision.comment?.trim()].filter(Boolean).join(" ");
  }).join("\n");
}
...
export function renderUi(data: ReviewUiData, requestToken: string) {
...
function day(){return BOOT.data.days[dayIndex]}function key(){return day()?'bingtang-review-'+day().date:''}function load(){const d=day();if(!d)return;try{states=JSON.parse(localStorage.getItem(key())||'{}')}catch{states={}};for(const r of d.review.articles){if(states[r.index])continue;if(['approved','rejected','held'].includes(r.status))states[r.index]={action:r.status,tag:r.reason_tag||'',comment:r.comment||''}}}
...
function el(tag,cls,text){const n=document.createElement(tag);if(cls)n.className=cls;if(text!==undefined)n.textContent=text;return n}function safeLink(url){try{const u=new URL(url);return ['http:','https:'].includes(u.protocol)?u.href:''}catch{return''}}function appendSources(card,label,items,exclude=[]){const rows=(items||[]).filter(src=>src&&src.name&&!exclude.some(root=>root&&root.name===src.name&&(!root.url||!src.url||root.url===src.url)));if(!rows.length)return;const sources=el('div','sources',label+' ');for(const src of rows){const href=safeLink(src.url||'');if(href){const link=el('a','',src.name);link.href=href;link.target='_blank';link.rel='noreferrer';sources.append(link)}else sources.append(el('span','',src.name+' '))}card.append(sources)}
function proposalFor(r){const all=day().revisions||{};const id=String(r.article_id||r.index);const bucket=(all.articles&&all.articles[id])||all[id]||all.articles?.find?.(x=>String(x.article_id||x.index)===id);if(!bucket)return undefined;return bucket.proposals?.find?.(p=>!r.pending_proposal_id||p.id===r.pending_proposal_id||p.proposal_id===r.pending_proposal_id)||bucket.pending_proposal||bucket.proposal||bucket}
function appendProposal(card,r){const p=proposalFor(r);if(!p)return;const box=el('div','proposal');box.append(el('h3','', '修正案（元の記事はまだ変更していません）'));if(p.instruction)box.append(el('p','','指示: '+p.instruction));if(p.summary)box.append(el('p','',p.summary));const changes=p.trace?.changes||p.changes||[];const list=el('div','');list.append(el('b','','変更する箇所'));for(const change of changes){const row=el('p','',humanField(change.field)+': '+preview(change.before)+' → '+preview(change.after));if(change.reason)row.append(el('small','','（'+change.reason+'）'));list.append(row)}box.append(list);box.append(el('p','','変更しない部分: '+((p.untouched||[]).join('、')||'上記以外の本文・注目ポイント・ソース')));box.append(el('p','', '削除情報: '+((p.deleted_information||[]).join('、')||'なし')));if((p.evidence_urls||[]).length){box.append(el('b','','根拠URL'));for(const url of p.evidence_urls){const href=safeLink(url);if(href){const a=el('a','',href);a.href=href;a.target='_blank';a.rel='noreferrer';box.append(a)}else box.append(el('p','',url))}}const actions=el('div','proposal-actions');[['apply','適用'],['apply_approved','適用して採用'],['discard','やめる'],['revision_requested','修正案への追加指示']].forEach(([action,label])=>{const b=el('button','soft',label);b.onclick=()=>setDecision(r.index,action,action==='revision_requested'?'':undefined);actions.append(b)});box.append(actions);if(states[r.index]?.action==='revision_requested')box.append(editor(r,true));card.append(box)}
function humanField(field){return ({title_ja:'タイトル',lead:'リード',what_happened:'何が起きたか',reaction_view:'反応・見られ方',why_it_matters:'注目ポイント',japan_context_note:'補足'})[field]||'本文'}function preview(value){const text=String(value||'空').replace(/\s+/g,' ').trim();return '「'+(text.length>60?text.slice(0,60)+'…':text||'空')+'」'}
function render(){dateSelect.innerHTML='';BOOT.data.days.forEach((d,i)=>{const o=el('option','',d.date);o.value=String(i);dateSelect.append(o)});if(!day()){app.innerHTML='';app.append(el('div','empty','今日のレビューはありません。'));document.querySelector('.top-actions').hidden=true;updateProgress();return}document.querySelector('.top-actions').hidden=false;load();app.innerHTML='';if(BOOT.data.warning)app.append(el('div','notice',BOOT.data.warning));day().review.articles.forEach((r,pos)=>app.append(card(r,pos)));updateProgress()}
  function card(r,pos){const a=articleFor(r),s=a.summary||{},c=el('article','card'+(pos===current?' active':''));c.id='article-'+r.index;const meta=el('div','meta');meta.append(el('span','chip badge',s.badge||'NEWS'),el('span','chip',s.category||a.raw.category||''),el('span','chip','確度'+(s.confidence||a.raw.reliability||'')));if(r.status==='revised_pending')meta.append(el('span','chip revision','🔄 修正版（'+(r.revision_count||1)+'回目）'));if(r.status==='proposal_pending'||r.pending_proposal_id)meta.append(el('span','chip revision','📝 修正案確認中'));if(r.status==='held')meta.append(el('span','chip held','保留'));if(r.publication?.published_at)meta.append(el('span','chip publication','公開済み'));else meta.append(el('span','chip','未公開'));c.append(meta,el('h2','',s.title_ja||r.title||a.raw.title),el('p','lead',s.lead||''));if(r.status==='revised_pending')c.append(el('div','previous','前回の指示: '+(r.comment||'（指示なし）')));const details=el('details','details'),sum=el('summary','','何が起きた？');details.append(sum,el('p','',s.what_happened||''));c.append(details);for(const section of s.detail_sections||[]){const detail=el('details','details'),heading=el('summary','',section.heading||'詳しく見る');detail.append(heading,el('p','',section.body||''));c.append(detail)}const reaction=typeof s.reaction_view==='string'?s.reaction_view.trim():'';if(reaction){const reactionDetails=el('details','details'),reactionSummary=el('summary','','反応・見られ方');reactionDetails.append(reactionSummary,el('p','',reaction));c.append(reactionDetails)}const box=el('div','comment-box');box.append(el('h3','','ビンタンの注目ポイント'),el('p','',s.why_it_matters||''));c.append(box);const supplement=typeof s.japan_context_note==='string'?s.japan_context_note.trim():'';if(supplement){const supplementBox=el('div','comment-box supplement-box');supplementBox.append(el('h3','','ビンタンからの補足'),el('p','',supplement));c.append(supplementBox)}appendSources(c,'ソース:',s.source_list||[]);appendSources(c,'関連角度のソース:',s.related_sources||[],s.source_list||[]);if(r.pending_proposal_id)appendProposal(c,r);if(r.status==='revised_pending'){const revert=el('div','proposal-actions');[['revert_previous','一つ前に戻す'],['revert_initial','初版に戻す']].forEach(([action,label])=>{const b=el('button','soft',label);b.onclick=()=>setDecision(r.index,action);revert.append(b)});c.append(revert)}const buttons=el('div','decision-buttons');[['approved','✅ 採用',''],['held','⏸ 保留','hold'],['revision_requested','✏️ 修正',''],['rejected','❌ 却下','reject']].forEach(([action,label,extra])=>{const b=el('button',extra,label);if(states[r.index]?.action===action)b.classList.add('selected');b.onclick=()=>setDecision(r.index,action);buttons.append(b)});c.append(buttons);if(['revision_requested','rejected','held'].includes(states[r.index]?.action)&&!r.pending_proposal_id)c.append(editor(r,false));return c}
function editor(r,proposal){const wrap=el('div','editor'),tags=el('div','tags');tags.append(el('small','', '理由タグ（任意）'));for(const tag of TAGS){const b=el('button',states[r.index]?.tag===tag?'selected':'',tag);b.onclick=()=>{states[r.index].tag=states[r.index].tag===tag?'':tag;save();render()};tags.append(b)}const ta=el('textarea');ta.placeholder=proposal?'追加指示（例: 初稿の雰囲気を維持してください）':states[r.index].action==='revision_requested'?'修正指示（理由タグなしで自由に入力できます）':'保留理由（任意）';ta.value=states[r.index].comment||'';ta.oninput=()=>{states[r.index].comment=ta.value;save()};wrap.append(tags,ta);return wrap}
function setDecision(index,action,comment){const old=states[index]||{};states[index]={action,tag:old.tag||'',comment:comment!==undefined?comment:old.comment||''};save();render();if(action==='approved')focusIndex(Math.min(day().review.articles.length-1,current+1))}function focusIndex(i){current=Math.max(0,Math.min(day().review.articles.length-1,i));render();document.getElementById('article-'+day().review.articles[current].index)?.scrollIntoView({behavior:'smooth',block:'center'})}
function updateProgress(){const total=day()?.review.articles.length||0,done=day()?day().review.articles.filter(r=>states[r.index]?.action).length:0;document.getElementById('progressText').textContent='今回送る判定 '+done+'/'+total;document.getElementById('progressBar').style.width=(total?done/total*100:0)+'%'}
function decisions(){return day().review.articles.map(r=>states[r.index]?{index:r.index,action:states[r.index].action,reasonTag:states[r.index].tag||undefined,comment:states[r.index].comment||''}:null).filter(Boolean)}function commentText(ds){return ds.map(d=>{const labels={approved:'採用',rejected:'却下',revision_requested:'修正',held:'保留',apply:'適用',apply_approved:'適用 採用',discard:'やめる',revert_previous:'一つ前に戻す',revert_initial:'初版に戻す'};return [d.index,labels[d.action],d.reasonTag,d.comment?.trim()].filter(Boolean).join(' ')}).join('\n')}
function validate(){const ds=decisions();if(!ds.length)return '送信する判定を1件以上選んでください。';const noInstruction=ds.filter(d=>d.action==='revision_requested'&&!d.comment?.trim());if(noInstruction.length)return '修正指示を入力してください: '+noInstruction.map(r=>r.index).join(', ');return''}
function confirm(){const error=validate();if(error){alert(error);return}const ds=decisions(),text=commentText(ds),counts={approved:0,revision_requested:0,rejected:0,held:0,apply:0,apply_approved:0,discard:0,revert_previous:0,revert_initial:0};ds.forEach(d=>counts[d.action]++);const labels={approved:'採用',revision_requested:'修正',rejected:'却下',held:'保留',apply:'適用',apply_approved:'適用して採用',discard:'やめる',revert_previous:'一つ前に戻す',revert_initial:'初版に戻す'};modal.hidden=false;modal.innerHTML='';const d=el('div','dialog');d.append(el('h2','','送信内容の確認'),el('p','',Object.entries(counts).filter(([,n])=>n).map(([name,n])=>labels[name]+' '+n).join('・')));const ta=el('textarea');ta.readOnly=true;ta.value=text;d.append(ta);const actions=el('div','dialog-actions'),cancel=el('button','soft','戻る'),copy=el('button','soft','コピー'),submit=el('button','primary','送信する');cancel.onclick=()=>modal.hidden=true;copy.onclick=()=>copyText(text,ta);submit.onclick=()=>submitReview(ds,text,submit);actions.append(cancel,copy,submit);d.append(actions);modal.append(d)}
...
dateSelect.onchange=()=>{dayIndex=Number(dateSelect.value);current=0;states={};render()};document.getElementById('approveRest').onclick=()=>{for(const r of day().review.articles)if(!states[r.index]&&['pending','revised_pending'].includes(r.status))states[r.index]={action:'approved',tag:'',comment:''};save();render()};document.getElementById('openConfirm').onclick=confirm;
document.addEventListener('keydown',e=>{if(e.ctrlKey&&e.key==='Enter'){e.preventDefault();confirm();return}if(['INPUT','TEXTAREA','SELECT'].includes(e.target.tagName))return;const r=day()?.review.articles[current];if(!r)return;if(e.key==='j'||e.key==='ArrowDown'){e.preventDefault();focusIndex(current+1)}if(e.key==='k'||e.key==='ArrowUp'){e.preventDefault();focusIndex(current-1)}if(e.key==='a')setDecision(r.index,'approved');if(e.key==='h')setDecision(r.index,'held');if(e.key==='e')setDecision(r.index,'revision_requested');if(e.key==='x')setDecision(r.index,'rejected');if(e.key==='z')document.getElementById('approveRest').click();if(/^[1-6]$/.test(e.key)&&states[r.index]&&['revision_requested','rejected','held'].includes(states[r.index].action)){states[r.index].tag=states[r.index].tag===TAGS[Number(e.key)-1]?'':TAGS[Number(e.key)-1];save();render()}if(e.key==='Enter')focusIndex(current+1)});render();
`;
