...
const TEMPLATE_COMMENT = /業界全体に影響を与える可能性|透明性向上につながる可能性|今後の動向(に|を)?(注目|注視|追|見守)|評価のポイントになりそう|新たな指標になるか|目が離せ(ない|ません)|注目したいところ|注目が集ま(りそう|る)/g;
...
async function main() {
  ...
  articles.filter((article) => article.summary).forEach((article, index) => {
    ...
    console.log(`- tone_mode: ${article.generationMeta?.tone_mode || "n/a"}`);
    console.log(`- exclamations: ${(comments.match(/[！!]/g) || []).length}`);
    ...
    console.log(`- hedging_mitai: ${(comments.match(/みたい|のようです/g) || []).length}\n`);
  });
