...
最終更新: 2026-07-18（Phase 3b 設計＋レビューゲート設計の完了時点）
...
3. **Phase 3b — 設計完了（2026-07-18・Fable）**: 読者ファースト品質修正。**Codex 実装指示書 = `docs/design-phase3b-reader-first-quality.md`**（B1〜B12）。
...
3b-R. **人間レビューゲート — 設計完了（2026-07-18・Fable）**: 出力安定まで、毎日の生成結果を GitHub Issue 上で採用／修正（コメント付き再生成）／却下に仕分けてから公開する。却下理由はタグ付きで `data/review-feedback.jsonl` に蓄積し、選定ロジック・プロンプト改訂の実測材料にする。**Codex 実装指示書 = `docs/design-review-gate.md`**（R1〜R7）。⚠️ は無し。3b と独立・先行導入可（公開の安全網になるため 3b 実装前の導入を推奨）。受け入れ基準の要点: Issue 1コメントで判定完結／承認分のみ公開／未判定日はサイト据え置き／REVIEW_GATE=false で従来挙動
