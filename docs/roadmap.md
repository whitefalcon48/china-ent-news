...
最終更新: 2026-07-14（Phase 4 サイト化＋Xbot化の設計完了時点）
...
- 未決事項: サイト名・キャラ名（design-phase4-character.md の推奨案を承認するか判断）/ X 課金の一次確認 / LICENSE・引用ポリシー整備 /「タイトル未設定」生成バグ修正（4a 前）
- キャラクター設計: 再検討完了・ユーザー承認待ち（2026-07-15）。設計正本 `docs/design-phase4-character.md`。
  推奨: 冰糖（ビンタン）／ビンタンちゃんニュース／です・ます調（Phase 3a 仮の少女口調は廃止提案）。
  承認後の反映手順（C1〜C6）は同文書に記載。旧A案（ミーシュ/パンダ）は棄却
