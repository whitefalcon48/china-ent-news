...
最終更新: 2026-07-12（2-3c 起票・Phase 3 方向確定時点）
...
#### 2-3c. hotfix（priority クランプ / unknown 除外 / URL 復元 / Serper 実測）— 🔜 次
...
### Phase 3: 内容ブラッシュアップ（生成パイプライン再設計）— 方向確定（2026-07-12）

方向確定の記録: `docs/design-phase3-content-pipeline.md`（Fable 検証済み）。
2-3c 完了後に着手。順序は固定:

1. ⚠️ **Fable 設計セッション**（1回で完結）: 事実台帳 JSON スキーマ / 禁止語リストと
   ゲート・警告の線引き / 劣化ポリシーと呼び出し総数上限 / editorial-character.md 改訂文面。
   終了時に Phase 3a の Codex 実装指示書を作成する
2. **Phase 3a**: 事実台帳の抽出 + 台帳からの執筆（2段階化）。本文圧縮・媒体名帰属もここで
3. **Phase 3b**: 秘書コメント分離 + 機械検査（trace 記録のみ → 観察 → ゲート化）+ 劣化ポリシー
...
後退がないことを確認してから次へ。2-4 の扱い（3a/3b へ吸収するか）は設計セッションで判断する。
