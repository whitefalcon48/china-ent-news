...
3b-R2. **ローカルレビューUI ＋ ビンタン口調v2 — 設計完了（2026-07-18・Fable）**: 毎朝の判定を `npm run review:ui`（ローカルWeb UI・ボタン中心・5〜10分以内）で行う主運用に変更。判定は Issue コメントに機械組み立てして gh CLI で投稿し、review-apply 以降は 3b-R の仕組みを無変更で再利用。Issue 直接コメントはフォールバックとして維持。あわせてビンタン口調を v2（丁寧語＋短いくだけた感想、「！」2〜4個・0個は再生成対象、sober 0個維持）へ改訂。**Codex 実装指示書 = `docs/design-review-ui-tone-v2.md`**（U1〜U7）。⚠️ は無し（文面確定済み）。実装順: R1〜R7 → U1〜U7。3b（B1〜B12）実装時は B1・B7-2・B8 を本設計の v2 文面で置き換える（U6）。
