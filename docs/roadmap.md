...
最終更新: 2026-08-07（週末公開UI調整の実装時点）

## 2026-08-07 実装記録（週末公開UI調整）

- ✅ トップフィードの「Xでシェア」を別タブで開くようにし、全記事共通の1200×630 PNGを使うOGP／X Cardメタを補強。ビンタンの注目ポイントには通常顔とウインク顔を記事ごとに決定的に使い分け、補足には集中顔を固定した。`npm run check`、`npm run test:site-feed`、`npm run build:site` が成功し、ビルド成果物への3画像コピー（4日分・26記事・33ページ）を確認。
...
- Actions配線hotfix（2026-07-19）: 生成ステップへ `SERPER_API_KEY` を渡すenv設定の欠落を修正。実APIでの `expansion success > 0` は再実測待ち。
...
2. ✅ **Phase 3a — 完了（2026-07-18 Actions実測: ledger_used 10/10・budget 24/45・gate 0件/warning 13件・官庁比率20.2%・媒体fresh 34件・複数ソースtopic 1件・最終出力10本）**: 事実台帳の抽出 + 台帳からの執筆（2段階化）+ 機械検査（ゲート2種+warning記録）
...
3. ✅ **Phase 3b — 実装完了・Actions待ち（2026-07-18）**: 読者ファースト品質修正。**Codex 実装指示書 = `docs/design-phase3b-reader-first-quality.md`**（B1〜B12）。
   内容: コメント分離工程（ビンタンのトーン修正・要するに構造・トーンモード）／用語解説の台帳強化（terms拡張）／表記辞書（config/terminology.json、広電局優先表記）／情報完全性ゲート（大哥型の生成前除外・10本未満許容）／claimCheck ゲート化（台帳外の金額・日付、反応捏造、未確認推測、定型句）／budget 45→60。
   ⚠️ は無し（設計・プロンプト文言は確定済みのため Codex で実装してよい。プロンプト文言は design-phase3b に確定済み・変更しない）。
   受け入れ基準の要点: 通常記事コメントに「！」2〜3個・sober 0個／中心用語の説明あり／広電局表記／information_incomplete 除外が trace に残る／10本未満許容／台帳外の金額・日付が公開されない／維持ライン後退なし
   実装・ローカル実測: check・全ダミー検証・フル実行成功。2026-07-18保存候補で大哥型3件の information_incomplete 発火を確認、trace budget limit 60 / used 0（ローカル取得失敗のため出力品質・維持ラインはActions確認待ち）。
3b-R. ✅ **人間レビューゲート — 実装完了・Actions待ち（2026-07-18）**: 出力安定まで、毎日の生成結果を GitHub Issue 上で採用／修正（コメント付き再生成）／却下に仕分けてから公開する。却下理由はタグ付きで `data/review-feedback.jsonl` に蓄積し、選定ロジック・プロンプト改訂の実測材料にする。**Codex 実装指示書 = `docs/design-review-gate.md`**（R1〜R7）。⚠️ は無し。受け入れ基準の要点: Issue 1コメントで判定完結／承認分のみ公開／未判定日はサイト据え置き／REVIEW_GATE=false で従来挙動
   実装・ローカル実測: reviewState・コメント文法・Issue本文・承認フィルタ・修正指示注入・feedback集計のダミー検証成功。実Issue作成〜クローズはActions確認待ち。
   Hotfix（2026-07-18）: 保存済み日付の再実行で persist の上書き防止により review.json が欠落した場合、保存済み articles JSON から順序を保ってレビュー状態だけを初期生成する処理を追加。初回生成・2回目再利用のダミー検証成功。
3b-R2. ✅ **ローカルレビューUI ＋ ビンタン口調v2 — 実装完了・Actions待ち（2026-07-18）**: 毎朝の判定を `npm run review:ui`（ローカルWeb UI・ボタン中心・5〜10分以内）で行う主運用に変更。判定は Issue コメントに機械組み立てして gh CLI で投稿し、review-apply 以降は 3b-R の仕組みを無変更で再利用。Issue 直接コメントはフォールバックとして維持。あわせてビンタン口調を v2（丁寧語＋短いくだけた感想、「！」2〜4個・0個は再生成対象、sober 0個維持）へ改訂。**Codex 実装指示書 = `docs/design-review-ui-tone-v2.md`**（U1〜U7）。
   ローカル実測: `npm run check`・fetch 3ケース・build→parse・口調v2 5ケース成功。dry-runで10件／修正版1件を1列表示、gh未導入時のセットアップ案内を確認。Issue投稿〜review-applyはActions確認待ち。
3b-V3. ✅ **選定品質・コメント品質 v3 P0 — 実装・当日判定・公開完了（2026-07-19／翌日2日連続確認待ち）**: 2026-07-19 の品質後退（前日topic再掲7/10・「要するに」開始9/10・公式のみ3本・レビュー却下6/10）への立て直し。公開判断専用の編集価値スコア（5軸10点・**7点以上のみ公開**・単一ソースは原則上限6点・official-only 1日1本）、過去5日の掲載履歴とレビュー結果による重複制御（実質的更新の定義・cooldown）、ソース拡張のゲート後移動（上位8件×2クエリ・SKIP_RSSHUB）、コメント生成v3（「要するに」削除・5角度・書き出し重複と本文言い換えの機械検査）、thin_unknown 誤爆修正、final_fill の上限迂回修正、出力本数可変。**Codex 実装指示書 = `docs/design-selection-quality-v3.md`**（P1-1〜P1-5 は後日・P0 の2日連続受け入れ後）。⚠️ は無し。受け入れ: 当日判定セット（公開全topic 7点以上／前日topic再掲0／official-only≦1／書き出し重複0／出力本数可変 ほか）→ 翌日実行で2日連続項目（expansion success>0・複数ソースtopic数など）を閉じる
   ...
   2026-08-01 hotfix: 7月21〜27日の7日間で6日が0件（各日候補101〜105件、EVS最高6点）だったため、`REVIEW_GATE=true` かつ7点合格0件の日だけ6点候補を最大3件レビュー対象へ救済。7月27日trace（候補103件・EVS 0/20合格・最高6点）で救済発動を回帰確認。`npm run check`・`npm run test:evs`成功。自動公開時・7点合格あり・5点以下では発動しない。
3b-V4. ✅ **コメント品質 v4（注目ポイント一本化・表記正規化・事実ゲート強化・モデル移行）— 実装完了・Actions待ち（2026-07-20）**: **Codex 実装指示書 = `docs/design-comment-quality-v4.md`**（Q1〜Q14）。
   ...
   2026-08-01 全体フィードバック: 公開コメント名は「ビンタンの注目ポイント」1枠を維持し、旧「ひとこと」と同じ口調を明示。固有名詞・中国業界用語の変換除外を廃止し、全公開テキストへ OpenCC cn→jp とプロジェクト固有マップを適用。7/24分は既存レビューIssueの全3記事を再生成し、字体残存0・editor_comment空を確認。
...
- Phase 4a UIユーザー確認: ✅ **承認済み（2026-07-17）**。確定: PCも1列／トップは最新10件＋アーカイブ／
  ...
  PC・スマホモック承認済み。
- Phase 4a 実装: ✅ **実装完了・Actions待ち（2026-07-17・Codex）**。U1〜U5（articles JSON、静的サイト、
  data永続化、GitHub Pages、Xダイジェスト）と公開用バスト／アバター／favicon／既定OGPを実装。
  実測: 2026-07-13の実データ10本から14ページ生成、内部リンク切れ0、1280px/375px横スクロール0、
  一覧10カード=注目ポイント枠10・小アバター10・タイトル横0、個別=統合枠1・アバター1、X文面244/280。
  ソース行空はbuild失敗、素材欠落fallback、0件日の空状態、公式のみ警告4件を確認。次はpush後のActions/Pages受け入れ確認。
  公開フローはレビューゲート導入後、承認記事のみのビルドに変わる（`docs/design-review-gate.md`）。
- Phase 4a 公開後UI／ソース品質修正: ✅ **実装完了（2026-07-19・Codex）**。トップ／日次カードを
  `<details>` 展開式へ変更し、注目ポイント全文表示、個別ページを共有用導線に整理。スマホ吹き出し位置修正、
  ヘッダータグラインと公式一次情報への警告を削除。公開データ実測で王年将成 9→4ソース、酷劇場 5→2ソースへ整理し、
  検索ページ・プレイリスト・別件CCTV動画・偽装ドメインを除外。検索拡張traceに除外件数／理由を追加。
  Actions `29677653473` 成功、公開HTML 200・折りたたみ10件・全文コメント10件・公式警告0件・確認済み不良リンク0件。
- X bot 半手動対応: ✅ **実装完了（2026-07-17・Fable）**。**運用手順の正本 = `docs/x-bot-operations.md`**。
  方針: まず半手動（Actions Summary の文面を人が X の予約投稿に保存・API キー不要）→ 品質担保後に
  `X_POSTING_ENABLED=true` で自動化。字数カウントを X 実仕様（CJK=2半角・URL=23半角・上限280）に置き換え、
  ダイジェスト見出しの切り詰めを解消（2026-07-13 実データで3本とも全文、244/280）。個別投稿候補
  （テキストのみ・4b 方式）を `src/site/xPostTexts.ts` で機械組み立てし、dry-run 時に
  `output/x_posts_YYYY-MM-DD.md` + Actions Summary + artifact `x-post-texts` へ出力。`npm run check` 通過。
  残: @bingtang_chan アカウント作成（ユーザー）／数日の品質確認／X API 単価の一次確認（手順書 §3）
  X 投稿文面の生成タイミングはレビューゲート導入後、レビュー完了時（承認分のみ）に移動する。
