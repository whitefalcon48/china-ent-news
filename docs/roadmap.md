...
3b-R2. **ローカルレビューUI ＋ ビンタン口調v2 — 設計完了（2026-07-18・Fable）**: 毎朝の判定を `npm run review:ui`（ローカルWeb UI・ボタン中心・5〜10分以内）で行う主運用に変更。判定は Issue コメントに機械組み立てして gh CLI で投稿し、review-apply 以降は 3b-R の仕組みを無変更で再利用。Issue 直接コメントはフォールバックとして維持。あわせてビンタン口調を v2（丁寧語＋短いくだけた感想、「！」2〜4個・0個は再生成対象、sober 0個維持）へ改訂。**Codex 実装指示書 = `docs/design-review-ui-tone-v2.md`**（U1〜U7）。⚠️ は無し（文面確定済み）。R1〜R7・B1〜B12 は実装済みのため実装対象は U1〜U7 のみ。U6 は適用済みの B1・B7-2・B8（editorial-character.md／buildBingtangCommentPrompt／runCommentCheck）を v2 文面・検査へ更新する差分適用。
