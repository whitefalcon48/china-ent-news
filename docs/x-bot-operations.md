...
最終更新: 2026-08-14。段階導入の方針: **まず半手動**（生成文面を人が X の予約投稿に保存）で品質を確認し、
担保できてから自動投稿（`X_POSTING_ENABLED=true`）へ切り替える。

アカウント開始時のアイコン／ヘッダー比較プロンプトと公開後通知フローは [`x-account-launch.md`](x-account-launch.md) を参照する。

文面の基礎制約は `docs/design-phase4-site.html` §3（ハッシュタグなし・絵文字はキャラ1つまで）、現行のひとこと契約は `docs/x-account-launch.md` §4。
...
2. レビューIssueの公開完了返信（GitHub通知メールでも受信可能）を開く。
   「X投稿文面 YYYY-MM-DD」として日次ダイジェストと個別投稿候補が字数付きで表示される
   （同じ内容がActions Summaryとartifact `x-post-texts`にもある。ローカルなら `npm run post:x` のdry-runでも生成可能）
...
- [ ] `why_it_matters`の先頭1文を再利用した「ビンタンのひとこと」が、記事の注目点と一致しているか
      ※X専用の新規生成は行わない。文面が崩れていたら公開コメントまたはテンプレ側の問題
