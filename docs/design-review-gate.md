...
- 各行の先頭に箇条書き記号（`-` / `・` / `*`）を付けない。例: `1 修正 口調 もっと明るく`
- 1行1判定。正規表現: `^(\d+)[\s　]+(採用|却下|修正)(?:[\s　]+(選定|口調|用語|事実|構成|その他))?(?:[\s　]+(.*))?$`
- `残り採用` / `全部採用`: 未判定（pending / revised_pending）をすべて採用
- `救済再生成`: review.json の記事が0本のときだけ実行する。保存済みの topic_candidates と selection_trace にあるEVS評価を再利用し、7点以上がない日のEVS 6点候補を最大3本まで選ぶ。過去日ですでにレビュー対象になった同一topicは除外する。保存evidenceを復元して台帳抽出・claim check・簡体字検査を通過した全候補だけを articles / fact ledger / review.json に書き戻し、status は pending とする。1件でも生成に失敗した場合は0件状態を維持してIssueへ診断を返信する。採用されるまで公開しない。
