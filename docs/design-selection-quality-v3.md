...
**実質的更新の定義（P0・deterministic）**: 履歴一致 topic に対し、①履歴に無い新しい evidence URL が存在し、②その evidence に公開日があり freshness が today/yesterday/recent で、かつ③その新 evidence のタイトル/key_points が次のいずれかに該当する。Serper の検索結果は公開日を取得していないため、ソース補強には使っても実質的更新の判定には使わない:
...
- **Serper endpoint**: Search API は `POST https://google.serper.dev/search` を使う。旧設計書の `api.serper.dev` は実測で HTTP 404 となるため使用しない
