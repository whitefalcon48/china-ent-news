...
2026-07-19 の実出力（10本中7本が前日と同一topic・「要するに」開始9本・公式のみ3本・レビューで6/10却下）の原因分析に基づく立て直し設計。公開判断専用の「編集価値スコア（EVS）」10点制・7点以上のみ公開、過去掲載履歴とレビュー結果による重複制御、ソース拡張の再配置、コメント生成v3を導入する。この文書に沿って Codex が実装する（V1〜V12）。設計判断はすべて確定済み。R1〜R7・B1〜B12・U1〜U7 は実装済みが前提。
...
| 軸 | 内容 | 判定 | 配点基準 |
|---|---|---|---|
| A 新規性・実質的更新 | 過去掲載との関係 | deterministic | 履歴に一致なし: freshness today/yesterday=2、recent=1。履歴一致あり: 強い更新（§4の official_decision / principal_response / result）=2、弱い更新（new_numbers のみ）=1、更新なし=**採点前に除外**（dup_no_update） |
...
- 功夫女足偷票房疑云（07-18掲載・新数字/新展開あり）: A=1〜2 で再掲可（実レビュー承認と一致）
...
**新しい永続ファイルは作らない**。実行時に `data/YYYY-MM-DD/`（過去5日分）の `review.json`・`articles_*.json`・`fact_ledger_*.json` から履歴をメモリ上に組み立てる（新規 `src/publicationHistory.ts`）。日付ディレクトリやファイルが無い日はスキップ（graceful・後方互換）。

履歴エントリ（メモリ上）: `{ date, topic_key, title, status(approved/rejected/revision_requested/pending), reason_tag, entities(works/people), topic_type, evidence_urls, numbers(台帳claims由来), opening(why_it_mattersの書き出し10字) }`

**同一topic判定**: `src/topicKey.ts` に純関数 `areTopicsLikelySame(a, b)` を追加（topicKey ロジックの一元管理原則に従いここへ置く）。判定 = 正規化 topic_key の完全一致、**または**（works∪people の entity 交差が1以上 **かつ** topic_type が一致）。タイトル表記の違い（功夫女足偷票房 ↔ 功夫女足偷票房疑云）は同一と判定される。表記違いは「続報」の根拠にしない。

**実質的更新の定義（deterministic）**: 履歴一致 topic に対し、①履歴に無い新しい evidence URL が存在し、かつ②その新 evidence のタイトル/key_points が次のいずれかに該当する:
...
- `new_numbers`(弱): 新 evidence の数字トークン（claimCheck の正規化を再利用）が履歴の numbers に無い

新 evidence が無い、または上記に該当しない場合は「更新なし」。

**cooldown（レビュー結果の反映）**:

| 履歴 status | 再選定条件（履歴window 5日） |
|---|---|
...
topicCandidates（seed・既存gate入力）
 ...
 → 履歴照合・cooldown ゲート（§4。dup_no_update / cooldown_rejected / in_review を除外）
 → ソース拡張（§6。位置をここへ移動: ゲート通過済みショートリストが対象）
...
- **位置**: 情報完全性ゲート・履歴ゲートの**後**、EVS 算定の**前**に移動する（現在は候補生成直後・選定前の先頭3件）。拡張結果が B/C 軸の入力になり、「選定される topic に拡張が効く」構造にする
- **対象**: ゲート通過候補を暫定順位（A軸スコア → japan_gap ヒューリスティック → newsworthiness_score）で並べ、**上位8件**（env `SOURCE_EXPANSION_MAX_TOPICS`、既定を 3→8 に変更）。クエリは topic あたり**2本**（env `SOURCE_EXPANSION_QUERIES_PER_TOPIC`、既定 1→2）。Serper 無料枠2,500/月に対し 8×2×31日=496/月で十分収まる
...
- `comment_paraphrase`: why_it_matters の各文（15字以上）について、本文（lead + what_happened）に対する4字シングル包含率が 0.55 以上なら「言い換え文」。言い換え文が全体の半数以上、または先頭文が言い換え → コメントのみ再生成1回 → 再生成後も該当なら warning のまま採用。閾値は定数化し report:quality で観測して調整する
...
publication_history: {
  ...
  matches: [{ topic_key, matched_date, matched_key, matched_status, reason_tag,
              substantive_update: "none" | "official_decision" | "principal_response" | "result" | "new_numbers",
              update_evidence_urls: [...], decision: "reselect_allowed" | "dup_no_update" | "cooldown_rejected" | "in_review" }]
}
...
- `topic_selection.dropped` の理由に追加: `dup_no_update` / `cooldown_rejected` / `in_review` / `evs_below_threshold` / `official_only_limit`
- expansion は既存項目のまま（試行・成功・追加evidence数）＋ ショートリスト対象の topic_key 一覧を追加

## 9. 実装タスク（V1〜V12。各ステップ後に `npm run check`）
...
- **V2** 掲載履歴モジュール（新規 src/publicationHistory.ts）: data/ 過去5日から履歴構築。ファイル欠落日はスキップ。受け入れ: 2026-07-18/19 の実データから履歴が組み上がり、entities・numbers・status が入る
- **V3** 同一topic判定（src/topicKey.ts に areTopicsLikelySame 追加）＋実質的更新判定＋cooldown ゲート（src/index.ts）。受け入れ: 07-18履歴 × 07-19候補の再現で、功夫女足偷票房疑云=更新あり再選定可、微短剧大赛・电视剧发行许可=dup_no_update、袁娅维（07-19に却下想定の履歴を与えた場合）=cooldown_rejected
- **V4** ソース拡張の再配置（src/index.ts, src/expandSources.ts）: ゲート後ショートリスト上位8件×2クエリ、SKIP_RSSHUB env、転載類似フィルタ。受け入れ: モックで「選定候補にのみ拡張が走る」「同内容転載が独立ソースに数えられない」「全滅でも続行」
- **V5** EVS deterministic 軸（新規 src/editorialValue.ts）: A/B/C・cap・例外・理由文字列。受け入れ: §2 の 07-19 検算5ケースがダミーで再現
...
- **V12** ローカル検証（§10）→ roadmap 更新（実測値1行）→ コミット
...
2. リプレイ検証（本設計の要）: 保存済み `data/2026-07-18` を履歴、`data/2026-07-19` の topic_candidates を入力として V1〜V7 を通し、§2・§4 の検算ケース（功夫女足偷票房=再選定可 / 微短剧大赛・电视剧发行许可・红色微短剧展=dup_no_update / 留几手=cap6 / 备案=例外昇格+official枠 / 周星驰路演哽咽=ゲート通過）を確認（scratchpad スクリプト・コミットしない）
