...
### ⚠️ Sol 案件は着手せず必ず止まる（重要）

タスクに着手する前に、roadmap の該当タスクに **⚠️ マーク**（「Sol に依頼推奨」等）が
付いていないか確認する。付いている場合、またはそのタスクが以下のいずれかに当たる場合は、
**自分で実装せず、作業を止めてユーザーに次のように知らせる**:

> このタスク（2-x）には Sol 推奨部分（◯◯）が含まれます。
> ここは出力品質を左右する要所なので、Sol に依頼することをおすすめします。
> Sol以外で進める部分（△△）だけ先にやることもできますが、どうしますか？

Sol に投げるべき合図（roadmap のモデル振り分け指針と一致）:
